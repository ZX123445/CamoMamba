...
from lib.CamoMamba import Network                   #先对输入进行DWT(小波变换+Mamba+小波反变换)，再进行特征聚合  本方法
# from lib.xunlian_Mamba_DWT import Network         #只对最高两层进行Mamba
# from lib.xunlian_4_Mamba_DWT import Network       #先对较高的4个输入进行Mamaba， 再进行DWT，进行特征聚合
# from lib.xunlain_4mamba_dwt import Network          #先对较高的4个输入进行Mamaba,再和原输入进行拼接，最后进行DWT，进行特征聚合
...
def val(test_loader, model, epoch, save_path, writer):
    ...
    with torch.no_grad():
        for i in range(test_loader.size):
            image, gt, _, _, _ = test_loader.load_data()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batchsize', type=int, default=2, help='training batch size') #8
    ...
    parser.add_argument('--train_root', type=str, default='C:/Users/Lenovo/Desktop/COD/TrainDataset/',
                        help='the training rgb images root')
    parser.add_argument('--val_root', type=str, default='C:/Users/Lenovo/Desktop/COD/TestDataset/',
                        help='the test rgb images root')
    parser.add_argument('--save_path', type=str,
                        default='./weight_DWT(Mamba)/',
